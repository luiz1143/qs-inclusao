...
// Definir variáveis e inicializar com valores do artigo
$title = $article["title"];
$date = $article["date"];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Verificar os erros de entrada antes de atualizar no banco de dados
    if (empty($title_err) && empty($date_err) && empty($excerpt_err) && empty($content_err)) {
        
        // Atualizar o artigo
        if (updateArticle($conn, $id, $title, $date, $excerpt, $content)) {
            // Obter o artigo atualizado
            $article = getArticleById($conn, $id);
...
<head>
    ...
    <title>Editar Artigo - Sistema de Gerenciamento de Blog</title>
...
<body>
    ...
    <main class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . "?id=" . $id; ?>" method="post">
            ...
            <div class="form-group">
                <input type="submit" class="btn btn-primary" value="Atualizar Artigo">
            </div>
        </form>
    </main>
